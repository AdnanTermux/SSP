...
function deliverSmsViaHttp(array $smsData): void {
    $pdo = getDB();
    ...
    // Format payload based on delivery format
    $headers = ['User-Agent: Sigma-SMS-A2P/1.0'];
    $postData = '';
    ...
    if (!empty($deliveryAuth)) {
        $headers[] = 'Authorization: ' . $deliveryAuth;
    }
    ...
    curl_setopt_array($ch, [
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_CUSTOMREQUEST => $deliveryMethod,
    ...
    $responseMsg = $success ? substr($response, 0, 500) : ($error ?: "HTTP $httpCode");
    
    $stmt = $pdo->prepare("
        INSERT INTO http_delivery_log (sms_received_id, number, delivery_url, success, http_code, response_message)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([
        $smsData['sms_id'],
...
function arrayToXml(array $data, $rootElement = 'sms'): string {
