...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrf()) {
        ...
        // Verify CAPTCHA
        if (empty($captcha) || !isset($_SESSION['math_captcha']) || (int)$captcha !== $_SESSION['math_captcha']['answer']) {
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                unset($_SESSION['math_captcha']); // Clear CAPTCHA
                redirect(APP_URL . '/dashboard.php');
            } else {
...
<head>
    ...
    <style>
        ...
        @keyframes captchaGlow {
            0%, 100% { box-shadow: 0 4px 12px rgba(79,70,229,.3); }
...
<div class="login-wrapper">
    <div class="login-card">
        ...
        <form method="POST" action="" id="loginForm" novalidate>
            ...
            <div class="mb-4">
                <label class="form-label" for="captchaInput">
                    <i class="ri-shield-check-line me-1"></i>Security Check
                </label>
                <div class="d-flex align-items-center gap-2 mb-2">
                    <div class="captcha-box">
                        <span class="captcha-num"><?= $_SESSION['math_captcha']['num1'] ?></span>
                        <span class="captcha-op">+</span>
                        <span class="captcha-num"><?= $_SESSION['math_captcha']['num2'] ?></span>
                ...
                <div class="input-group-glass">
                    <i class="ri-calculator-line input-icon"></i>
                    <input
                        type="number"
                        id="captchaInput"
                        name="captcha"
                ...
                <small class="text-muted" style="font-size:.75rem;">
                    <i class="ri-information-line me-1"></i>Solve the math problem to verify you're human
                </small>
            </div>
...
function refreshCaptcha() {
    fetch('<?= APP_URL ?>/ajax/refresh_captcha.php', {
        method: 'POST',
        headers: {
    ...
    .then(d => {
        if (d.status === 'success') {
            document.querySelector('.captcha-num:first-child').textContent = d.num1;
            document.querySelector('.captcha-num:last-of-type').textContent = d.num2;
